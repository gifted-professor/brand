import { LocalCliSettings } from './components/LocalCliSettings';
...
export default function App({ channelPreviewMode = false }: { channelPreviewMode?: boolean } = {}) {
  ...
  const [modal, setModal] = useState<'skills' | 'history' | 'help' | 'brief' | 'cli' | null>(null);
  ...
  return <div className={`unified-app${isNew ? ' is-new' : ''}${channelPreviewMode ? ' has-cocreation' : ''}`}>
    {channelPreviewMode ? (<header className="u-topbar"><button className="u-logo" type="button" aria-label="返回第一页" onClick={() => window.location.assign('/')}><img className="u-brand-symbol" src="/brand-relations/assets/mark-black.svg" alt="" /></button><div className="u-project-switch"><span className="u-project-title">{projectTitle}</span><small>{projectId ? '已有项目' : session?.mode === 'demo' ? '交互演示' : '联名创作空间'}</small></div><div className="u-header-actions"><button type="button" aria-label="选择本地 CLI 和模型" onClick={() => setModal('cli')} className={`u-connection ${runtime?.configured ? 'ready' : ''}`}><i />{runtime?.transport === 'grok-cli' ? 'Grok CLI · ' : runtime?.transport === 'codex-cli' ? 'Codex CLI · ' : ''}{runtime?.model || '选择本地 CLI'}</button><button title="项目简报与资料" aria-label="项目简报与资料" onClick={() => setModal('brief')}><FileText size={16} /></button><button title="工作方法" aria-label="工作方法" onClick={() => setModal('skills')}><Settings2 size={16} /></button>{session && <a className="u-export" aria-label="导出当前方案" href={`/api/sessions/${session.id}/export`}><ArrowDownToLine size={14} /><span>导出</span></a>}</div></header>) : (<header className="u-topbar"><div className="u-logo"><img className="u-brand-lockup" src="/brand-relations/assets/lockup-black.svg" alt="Brand Relations · 品牌联名工具" /><img className="u-brand-symbol" src="/brand-relations/assets/mark-black.svg" alt="Brand Relations · 品牌联名工具" /></div><div className="u-project-switch"><button onClick={() => void openHistory()}><span>{projectTitle}</span><ChevronDown size={14} /></button><small>{projectId ? '已有项目' : session?.mode === 'demo' ? '交互演示' : '联名创作空间'}</small></div><div className="u-header-actions"><button type="button" aria-label="选择本地 CLI 和模型" onClick={() => setModal('cli')} className={`u-connection ${runtime?.configured ? 'ready' : ''}`}><i />{runtime?.transport === 'grok-cli' ? 'Grok CLI · ' : runtime?.transport === 'codex-cli' ? 'Codex CLI · ' : ''}{runtime?.model || '选择本地 CLI'}</button><button title="项目简报与资料" aria-label="项目简报与资料" onClick={() => setModal('brief')}><FileText size={16} /></button><button title="工作方法" aria-label="工作方法" onClick={() => setModal('skills')}><Settings2 size={16} /></button><button className="u-new" aria-label="新建项目" onClick={newProject} disabled={busy}><Plus size={15} /><span>新建</span></button>{session && <a className="u-export" aria-label="导出当前方案" href={`/api/sessions/${session.id}/export`}><ArrowDownToLine size={14} /><span>导出</span></a>}</div></header>)}
    ...
    {modal === 'cli' && <Modal title="本地 CLI 与模型" onClose={() => setModal(null)}><LocalCliSettings onSelected={info => { setRuntime(info); setModal(null); setToast('本地 CLI 选择已保存，后续步骤将使用新配置。'); }} /></Modal>}
