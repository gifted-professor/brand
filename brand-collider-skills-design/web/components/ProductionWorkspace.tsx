...
import { resolveProjectStage, resolveSessionStage } from '../workflow-stage';
...
export function ProductionWorkspace({ session, projectId, onProjectLoaded, onDiscuss, focusNodeId, followRequest }: Props) {
  ...
  const displayed = useMemo(() => {
    ...
    if (!current?.nodes.length) return source;
    ...
    return { ...source, updatedAt: current.updatedAt, nodes, assets: [...current.assets, ...source.assets], edges: [...current.edges, ...source.edges], nodeCount: nodes.length, assetCount: current.assets.length + source.assets.length };
  ...
  return <div className="u-production-workspace">
    <ProductionCanvas project={displayed} refreshing={refreshing} onRefresh={() => void refresh()} onBack={() => {}} onDiscuss={onDiscuss} embedded activeLane={activeLane} focusNodeId={focusNodeId} followRequest={followRequest} />
