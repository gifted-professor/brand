...
这些映射沿用现有六个 Skill 身份与工具契约，规则版本为 1.6.2：资料稀少不阻断文字概念；需要准确品牌身份的视觉，必须以真实素材和实际附图为依据。运行时把 `agent/SYSTEM.md`、六个 Skill 及共享 `VISUAL_EVIDENCE.md` 正文纳入内容哈希，不能只加载链接。每个会话使用固定快照；主动刷新研究时新版本采用当前方法，并保留旧版本快照。本次没有改名或迁移完整业务契约。

## 素材筛选与主体核对（1.6.2）

网页来源先提取每页最多 64 条图片元信息，由素材筛选 Agent 选择与目标匹配的候选，再下载和实际看图；初始每品牌每轮最多 3 张，补采最多 2 张。此任务使用 brand-profile 方法，无新增创作 Skill。所需主体与实际主体分别保存，核对输出 targetMatch 和 assetType；官网公司 Logo 不等于角色资产。错误或缺失目标触发最多一次补找，每品牌、每采集阶段的两轮额度持久保存。身份绑定按 identityTargets 检查每件的资产类型与匹配状态；未匹配角色不能凭已有 Logo 放行。画布显示真实观察主体及待核对/不适用状态。
...
九步方案/物料流程完成后，对话支持“现在我希望基于这一套物料来生成一只宣传视频”。此入口不回退九步，不新增物料版本；在视频泳道创建独立结果节点。默认本轮规格为 Seedance 2.5、480p、16:9、约30秒。

宿主核对每件在范围内的产图及对应验收哈希，复用实际文件，并携带已绑定、身份匹配且来源哈希一致的角色/字标原图。概念效果图用于产品连续性，不被描述成实物照片。Flova 接收一次完整创作说明，负责依据已有物料建立故事、分镜、音效、视频和时间线；案例的五段节奏作为参考，没有对应物料的纸袋、赠品等不引入。模型、分辨率与比例是明确要求，服务端不支持时报告原因。

集成只保存 Flova 项目与任务句柄、源图哈希和实际下载文件，不维护第二套分镜状态。调用已发布的 `flova project create`、`upload` 和默认聚合 `run`；CLI 是唯一后端轮询方。实时预览经本地文件登记显示在画布。需要用户决定时展示原始待确认项，用户输入“选择视频选项N”后才发送匹配的 action/message/option；普通“继续”不会代替审批。

只有制作终态成功、无阻塞选项、export readiness 通过、export 成功且下载到非空 MP4 后，才标记“成片已保存”。这不替代用户对角色、标识及连续性的成片验看。中断后优先 `run result/current` 或 `export status/current` 恢复原任务，绝不因本地超时重发；未取得项目ID的模糊创建结果需要先在 Flova 核对。账号不足等条件展示 Flova 返回的操作页面。

实现：`src/server/flova-video.ts`（CLI 生命周期）、`ColliderRuntime.createVideo/continueVideo`（会话入口与锁）、`web/session-video.ts`（画布与文件）。使用当前默认 Flova 登录，不在网页或会话JSON中保存认证令牌。上传回包在服务端私有目录保留供CLI使用，成片通过当前版本的本地文件端点播放/下载。
