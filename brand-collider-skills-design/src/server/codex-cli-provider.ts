...
export function cliEnvironment(source: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
  return Object.fromEntries(['HOME', 'PATH', 'TMPDIR', 'LANG', 'LC_ALL', 'CODEX_HOME', 'SYSTEMROOT', 'USERPROFILE', 'SSL_CERT_FILE', 'SSL_CERT_DIR', 'HTTPS_PROXY', 'HTTP_PROXY', 'ALL_PROXY', 'NO_PROXY', 'https_proxy', 'http_proxy', 'all_proxy', 'no_proxy']
    .filter(key => source[key] !== undefined).map(key => [key, source[key]]));
}

/** Codex strict output schemas omit unsupported uniqueness checks; runtime validators retain them. */
export function codexOutputSchema(value: unknown): unknown {
  if (Array.isArray(value)) return value.map(codexOutputSchema);
  if (!value || typeof value !== 'object') return value;
  const schema = Object.fromEntries(Object.entries(value).filter(([key]) => key !== 'uniqueItems').map(([key, child]) => [key, codexOutputSchema(child)]));
  if (schema.type === 'object' && schema.properties && typeof schema.properties === 'object') {
    // All generated fields are explicit; nullable fields already encode missing data.
...
export class CodexCliProvider implements TextProvider {
  ...
  async complete(messages: ChatMessage[], task?: AgentTask): Promise<unknown> {
    ...
    try {
      ...
      await Promise.all([writeFile(schemaPath, JSON.stringify(codexOutputSchema(task.schema)), { mode: 0o600 }), writeFile(join(runDirectory, 'task.txt'), prompt, { mode: 0o600 })]);
      ...
      await new Promise<void>((resolveRun, reject) => {
        ...
        const stop = (error: RuntimeError) => { if (failure) return; failure = error; activity.stop(error.status === 499 ? 'cancelled' : error.status === 504 ? 'timeout' : 'exit'); kill('SIGTERM'); forceTimer = setTimeout(() => kill('SIGKILL'), 1000); };
        ...
        function line(raw: string) {
          ...
          activity.event({ type: event.type === 'turn.completed' || event.type === 'turn.failed' ? 'end' : event.type === 'thread.started' ? 'available_commands' : 'other' });
          ...
          if (event.type === 'error' || event.type === 'turn.failed') {
            ...
            if (message.includes('invalid_json_schema')) reportedError = '当前阶段输出格式不兼容 Codex，请检查服务端 JSON Schema。';
        ...
        child.stderr.on('data', chunk => activity.stderr(chunk));
