...
发现入口在服务端继承相邻工作台的模型与图片配置，自己的 `.env.local` 和进程环境变量可覆盖同名项；凭证不会写入前端或复制到新文件。运行数据保存在 `brand-relations-demo/outputs/`；原工作台会话保留在原位置。两入口共享代码，但会话存储分别维护。
...
进入共创画布后，“一键生成完整联名方案”带入双方资料，开启自动选案与九步工作流。支持原工作台的 Grok/Codex CLI 配置；具备自动制作能力时包含逐件出图，否则明确显示仅方案与提示词。

完整工作流使用单独的会话映射，不修改原预演简报；重复点击和重新进入会恢复已有完整会话。右侧固定显示九步进度，可展开逐步查看，并支持暂停和继续。九步完成与真实媒体完成状态沿用原工作台的实际记录。按钮点击前不执行生成请求。

新增回归覆盖：独立完整会话、并发重复打开与恢复、未连接模型阻断、无图像能力的文本流程、演示适配器完整走完九步。浏览器检查按钮与服务连接状态；未为界面验证触发真实批量生成。
